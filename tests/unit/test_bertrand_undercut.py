"""Test cases for Bertrand oligopoly simulation.

This module contains comprehensive tests for the Bertrand simulation,
including edge cases for undercutting, ties, and demand non-negativity.
"""

import pytest
import math
from src.sim.bertrand import bertrand_simulation, calculate_demand, allocate_demand
...
class TestBertrandUndercut:
    ...
    def test_undercut_to_marginal_cost(self):
        ...
        assert result.quantities[0] == pytest.approx(80.0, abs=1e-6)  # Q(20) = 100 - 1*20 = 80
    ...
    def test_undercut_below_marginal_cost(self):
        ...
        # Firm 0 should get all demand
        assert result.quantities[0] == pytest.approx(85.0, abs=1e-6)  # Q(15) = 100 - 1*15 = 85
        assert result.quantities[1] == pytest.approx(0.0, abs=1e-6)
        assert result.quantities[2] == pytest.approx(0.0, abs=1e-6)
...
class TestBertrandTie:
    ...
    def test_two_firms_tie_for_lowest_price(self):
        ...
        expected_demand_per_firm = (120.0 - 1.2 * 22.0) / 2  # Q(22) = 93.6, split equally = 46.8
    ...
    def test_three_firms_tie_for_lowest_price(self):
        ...
        expected_demand_per_firm = (100.0 - 1.0 * 15.0) / 3  # Q(15) = 85, split equally = 28.33...
        assert result.quantities[0] == pytest.approx(expected_demand_per_firm, abs=1e-6)
        assert result.quantities[1] == pytest.approx(expected_demand_per_firm, abs=1e-6)
...
class TestDemandNonNegativity:
    ...
    def test_demand_zero_when_price_too_high(self):
        ...
        alpha, beta = 50.0, 2.0
        costs = [10.0, 15.0]
        prices = [40.0, 35.0]  # Price 40 > alpha/beta = 25, so Q(40) = max(0, 50-2*40) = 0
