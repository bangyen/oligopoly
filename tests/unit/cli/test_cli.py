...
class TestCournotCLI:
    ...
    def test_cli_single_firm(self) -> None:
        ...
        with patch(
            "sys.argv",
            ["cournot", "--a", "50", "--b", "0.5", "--costs", "15", "--q", "20"],
        ):
            with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_cli_error_handling_negative_quantities(self) -> None:
        ...
        with patch(
            "sys.argv",
            ["cournot", "--a", "100", "--b", "1", "--costs", "10", "--q", "-5"],
        ):
    ...
    def test_cli_error_handling_invalid_costs(self) -> None:
        ...
        with patch(
            "sys.argv",
            ["cournot", "--a", "100", "--b", "1", "--costs", "10,abc", "--q", "10,20"],
        ):
            with patch("sys.stderr", new_callable=StringIO) as mock_stderr:
                with pytest.raises(SystemExit):
                    main()
    ...
    def test_cli_error_handling_invalid_quantities(self) -> None:
        ...
        with patch(
            "sys.argv",
            ["cournot", "--a", "100", "--b", "1", "--costs", "10,20", "--q", "10,xyz"],
        ):
            with patch("sys.stderr", new_callable=StringIO) as mock_stderr:
                with pytest.raises(SystemExit):
                    main()
    ...
    def test_cli_mismatched_lengths(self) -> None:
        ...
        with patch(
            ...
            with patch("sys.stderr", new_callable=StringIO) as mock_stderr:
                ...
                error_output = mock_stderr.getvalue()
                assert (
                    "Costs list length (2) must match quantities list length (3)"
                    in error_output
                )
    ...
    def test_cli_zero_price_scenario(self) -> None:
        ...
        with patch(
            "sys.argv",
            ["cournot", "--a", "100", "--b", "1", "--costs", "10,20", "--q", "50,50"],
        ):
            with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
    ...
    def test_cli_fractional_values(self) -> None:
        ...
        with patch(
            "sys.argv",
            ["cournot", "--a", "75.5", "--b", "1.25", "--costs", "12.5", "--q", "30.2"],
        ):
            with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                main()
                output = mock_stdout.getvalue()
