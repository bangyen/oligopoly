...
def test_idempotency_different_seeds(setup_database):
...
def test_idempotency_same_seed(setup_database):
    ...
    for round_idx in range(3):
        assert data1["firms_data"][0]["actions"][round_idx] == data2["firms_data"][0]["actions"][round_idx]
        assert data1["firms_data"][1]["actions"][round_idx] == data2["firms_data"][1]["actions"][round_idx]
        assert data1["rounds_data"][round_idx]["price"] == data2["rounds_data"][round_idx]["price"]


def test_idempotency_no_seed(setup_database):
...
def test_idempotency_bertrand_model(setup_database):
...
def test_idempotency_different_configs(setup_database):
...
def test_idempotency_multiple_firms(setup_database):
    ...
    for firm_idx in range(4):
        assert data1["firms_data"][firm_idx]["actions"][0] == data2["firms_data"][firm_idx]["actions"][0]
