...
from sim.models import Base, Run, Round, Result
from sim.runner import run_game, get_run_results


# Test database setup
SQLALCHEMY_DATABASE_URL = "sqlite:///./test_persistence.db"
...
def test_persistence_counts(setup_database):
    ...
    try:
        ...
        for round_idx in range(rounds):
            round_results = db.query(Result).filter(
...
def test_get_run_results_structure(setup_database):
...
def test_get_run_results_validation(setup_database):
...
def test_run_not_found(setup_database):
...
def test_bertrand_persistence(setup_database):
