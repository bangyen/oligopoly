...
class SimulationRequest(BaseModel):
    ...
    model: str = Field(..., pattern="^(cournot|bertrand)$", description="Competition model type")
    rounds: int = Field(..., gt=0, le=1000, description="Number of simulation rounds")
    params: Dict[str, Any] = Field(default_factory=dict, description="Market parameters")
    firms: List[FirmConfig] = Field(..., min_length=1, max_length=10, description="Firm configurations")
