...
from ..games.bertrand import (
    bertrand_simulation,
    parse_prices,
)
from ..games.bertrand import (
    parse_costs as parse_bertrand_costs,
)
from ..games.cournot import cournot_simulation, parse_costs, parse_quantities
...
Examples:
  ...
  bertrand --alpha 100 --beta 1.0 --costs 10,15 --p 12,14
    ...
    try:
        ...
        for i, (p, q, pi) in enumerate(
            zip(result.prices, result.quantities, result.profits)
        ):
