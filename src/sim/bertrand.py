...
def allocate_demand(prices: List[float], costs: List[float], alpha: float, beta: float) -> Tuple[List[float], float]:
    ...
    min_price_firms = [i for i, p in enumerate(prices) if math.isclose(p, min_price, abs_tol=1e-10)]
...
def bertrand_simulation(
    ...
    if len(costs) != len(prices):
        raise ValueError(f"Costs list length ({len(costs)}) must match prices list length ({len(prices)})")
